...
app.get("/profile", requireAuth, (req, res) => {
    ...
    try {
        ...
        const favCount = db.prepare("SELECT COUNT(*) as favs FROM favorites WHERE user_id = ?").get(user.id)?.favs || 0;
...
app.get("/favorites", requireAuth, (req, res) => {
    const user = req.session.user;
    try {
        const favorites = db.prepare(`SELECT product_id, added_at FROM favorites WHERE user_id = ? ORDER BY added_at DESC`).all(user.id);
        ...
        for (const fav of favorites) {
            ...
            const imagePath = productId.startsWith('product_') ? `/uploads/${item.image}` : `/photo/${item.image}`;
            const artist = productId.startsWith('product_') ? item.artist : 'Проигрыватель';
            
            productHTML += `
<div class="benefit" data-product-id="${item.id}" data-product-name="${escapeHtml(item.name)}" data-product-artist="${escapeHtml(artist)}" data-product-price="${item.price}" data-product-image="${imagePath}" data-product-description="${escapeHtml(item.description || 'Нет описания')}" data-product-genre="${escapeHtml(item.genre || 'Rock')}" data-product-year="${escapeHtml(item.year || '1970')}">
    <div class="image-container"><img src="${imagePath}" class="graf"><img src="/photo/plastinka-audio.png" class="plastinka"></div>
    <div class="benefit-info">
        <div class="album-nazv-container"><span class="album-nazv">${escapeHtml(item.name)}</span></div>
        <div class="album-title-container"><span class="album-title">${escapeHtml(artist)}</span></div>
        <div class="album-bottom"><span class="album-price">${item.price}$</span><form action="/add-to-cart" method="POST" class="add-to-cart-form"><input type="hidden" name="id" value="${productId}"><button type="submit" class="add-to-cart"><img src="/photo/b_plus.svg" class="cart-icon"></button></form></div>
    </div>
</div>`;
        }
        
        if (productHTML === '') productHTML = '<p style="text-align: center; color: #aaa; grid-column: 1/-1;">Нет избранных товаров</p>';
        
        const html = `<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>Избранное · Plastinka</title><link rel="stylesheet" href="/style.css"><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css"></head>
<body>
<header>
...
<section class="catalog-title-section"><h2 class="catalog-title">ИЗБРАННОЕ</h2></section>
<section class="benefits"><div class="benefits-grid">${productHTML}</div></section>
...
<script src="/main.js"></script>
        ...
        res.status(500).send("Ошибка загрузки избранного");
    }
});
...
function adminPanelHTML(productCount, playerCount, userCount, productsRows, playersRows, usersRows, username) {
    return `<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Админ панель · Plastinka</title>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
<style>
...
@media(max-width:768px){.admin-wrapper{padding:10px;}.stats-grid{grid-template-columns:repeat(2,1fr);}th,td{padding:8px;font-size:12px;}}
</style>
</head>
<body>
<div class="admin-wrapper">
<div class="admin-header"><div class="header-left"><h1><i class="fas fa-crown"></i> Админ панель</h1><p>Управление каталогом и пользователями</p></div><div class="header-right"><div class="admin-user"><i class="fas fa-user-shield"></i><span>${username}</span></div><a href="/" class="logout-link home-link"><i class="fas fa-home"></i> На сайт</a><a href="/logout" class="logout-link"><i class="fas fa-sign-out-alt"></i> Выйти</a></div></div>
<div class="stats-grid"><div class="stat-card"><div class="stat-value">${productCount}</div><div class="stat-label">📀 Пластинок</div></div><div class="stat-card"><div class="stat-value">${playerCount}</div><div class="stat-label">🎵 Проигрывателей</div></div><div class="stat-card"><div class="stat-value">${userCount}</div><div class="stat-label">👥 Пользователей</div></div><div class="stat-card"><div class="stat-value">${productCount + playerCount}</div><div class="stat-label">📦 Всего товаров</div></div></div>
<div class="action-buttons"><button class="btn-primary" id="addProductBtn"><i class="fas fa-plus-circle"></i> Добавить пластинку</button><button class="btn-secondary" id="addPlayerBtn"><i class="fas fa-plus-circle"></i> Добавить проигрыватель</button><a href="/admin/settings" class="btn-settings"><i class="fas fa-sliders-h"></i> Настройки главной</a></div>
<div class="tabs"><button class="tab-btn active" data-tab="products"><i class="fas fa-record-vinyl"></i> Пластинки</button><button class="tab-btn" data-tab="players"><i class="fas fa-headphones"></i> Проигрыватели</button><button class="tab-btn" data-tab="users"><i class="fas fa-users"></i> Пользователи</button></div>
<div id="products-tab" class="tab-content active"><div class="table-container"><table><thead><tr><th>Тип</th><th>Изображение</th><th>Название</th><th>Исполнитель</th><th>Жанр</th><th>Год</th><th>Цена</th><th>Рейтинг</th><th>Действия</th></tr></thead><tbody>${productsRows || '<tr><td colspan="9">Нет пластинок</td></tr>'}</tbody></table></div></div>
<div id="players-tab" class="tab-content"><div class="table-container"><table><thead><tr><th>Тип</th><th>Изображение</th><th>Название</th><th>Описание</th><th>Цена</th><th>Действия</th></tr></thead><tbody>${playersRows || '<tr><td colspan="6">Нет проигрывателей</td></tr>'}</tbody></table></div></div>
<div id="users-tab" class="tab-content"><div class="table-container"><table><thead><tr><th>Аватар</th><th>Имя</th><th>Роль</th><th>📝 Отзывы</th><th>❤️ Избранное</th><th>🛒 Корзина</th><th>Действия</th></tr></thead><tbody>${usersRows || '<tr><td colspan="7">Нет пользователей</td></tr>'}</tbody></table></div></div>
</div>
...
function openAddModal(type){document.getElementById('modalTitle').innerText=type==='product'?'Добавить пластинку':'Добавить проигрыватель';document.getElementById('itemType').value=type;document.getElementById('itemId').value='';document.getElementById('itemForm').reset();document.getElementById('itemModal').classList.add('active');}
document.querySelectorAll('.edit-product').forEach(btn=>btn.onclick=function(){editItem('product',this.dataset.id);});
document.querySelectorAll('.edit-player').forEach(btn=>btn.onclick=function(){editItem('player',this.dataset.id);});
function editItem(type,id){fetch('/admin/get-item',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:type,id:id})}).then(res=>res.json()).then(data=>{document.getElementById('modalTitle').innerText=type==='product'?'Редактировать пластинку':'Редактировать проигрыватель';document.getElementById('itemType').value=type;document.getElementById('itemId').value=id;document.getElementById('itemName').value=data.name||'';document.getElementById('itemArtist').value=data.artist||'';document.getElementById('itemGenre').value=data.genre||'';document.getElementById('itemYear').value=data.year||'';document.getElementById('itemPrice').value=data.price||'';document.getElementById('itemDescription').value=data.description||'';document.getElementById('itemModal').classList.add('active');});}
document.querySelectorAll('.delete-product').forEach(btn=>btn.onclick=function(){if(confirm('Удалить?'))deleteItem('product',this.dataset.id);});
document.querySelectorAll('.delete-player').forEach(btn=>btn.onclick=function(){if(confirm('Удалить?'))deleteItem('player',this.dataset.id);});
function deleteItem(type,id){fetch('/admin/delete-item',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:type,id:id})}).then(res=>res.json()).then(data=>{if(data.success)location.reload();});}
document.querySelectorAll('.edit-user').forEach(btn=>btn.onclick=function(){document.getElementById('editUserId').value=this.dataset.id;document.getElementById('editUsername').value=this.dataset.name;document.getElementById('editRole').value=this.dataset.role;document.getElementById('editPassword').value='';document.getElementById('userModal').classList.add('active');});
document.querySelectorAll('.delete-user').forEach(btn=>btn.onclick=function(){if(confirm('Удалить пользователя?'))deleteUser(this.dataset.id);});
function deleteUser(id){fetch('/admin/delete-user',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})}).then(res=>res.json()).then(data=>{if(data.success)location.reload();});}
document.getElementById('itemForm').onsubmit=function(e){e.preventDefault();fetch('/admin/save-item',{method:'POST',body:new FormData(this)}).then(res=>res.json()).then(data=>{if(data.success)location.reload();else alert('Ошибка');});};
document.getElementById('userForm').onsubmit=function(e){e.preventDefault();fetch('/admin/update-user',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:document.getElementById('editUserId').value,username:document.getElementById('editUsername').value,role:document.getElementById('editRole').value,password:document.getElementById('editPassword').value})}).then(res=>res.json()).then(data=>{if(data.success)location.reload();else alert('Ошибка');});};
</script>
</body>
</html>`;
}
